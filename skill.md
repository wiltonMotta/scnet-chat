...
3. **执行命令**（根据操作系统选择正确的启动脚本）：
   - **macOS/Linux**: `{baseDir}/scripts/run.sh "自然语言命令"`
...
{baseDir}/scripts/run.sh "查询作业"                      # 查询所有区域的实时作业
{baseDir}/scripts/run.sh "查看运行中的作业"              # 查询所有区域运行中的作业
{baseDir}/scripts/run.sh "历史作业"                      # 查询所有区域的历史作业
{baseDir}/scripts/run.sh "作业详情 12345"                # 查询特定作业详情（需指定区域）

# 作业管理 - 查询（高级筛选）
{baseDir}/scripts/run.sh "查询作业 第2页 每页20条"
{baseDir}/scripts/run.sh "查询队列 debug 的作业 所有字段"
{baseDir}/scripts/run.sh "查询用户 zhangsan 的作业"
{baseDir}/scripts/run.sh "查询组作业"
{baseDir}/scripts/run.sh "查询区域ID 12345 的作业"       # 筛选特定区域的作业

# 作业管理 - 历史作业查询（高级用法）
{baseDir}/scripts/run.sh "历史作业 开始时间 2024-01-01 00:00:00 结束时间 2024-01-31 23:59:59"
{baseDir}/scripts/run.sh "历史作业 最近7天"
{baseDir}/scripts/run.sh "查询最近三天的历史作业"                    # 中文数字支持
{baseDir}/scripts/run.sh "查询从2026-04-01到2026-04-03的历史作业"   # 简写日期范围
{baseDir}/scripts/run.sh "查询历史作业，每页5条，显示第二页"         # 中文页码
{baseDir}/scripts/run.sh "查询历史作业，状态为失败"                  # 中文状态筛选
{baseDir}/scripts/run.sh "查询历史作业，队列为comp，每页10条"

# 作业管理 - 实时作业查询（高级用法）
{baseDir}/scripts/run.sh "查询作业 开始时间 2024-01-01 00:00:00 结束时间 2024-01-31 23:59:59"
{baseDir}/scripts/run.sh "查询最近3天的作业"
{baseDir}/scripts/run.sh "查询运行中的作业，每页5条"
{baseDir}/scripts/run.sh "查询作业，状态为排队，显示第2页"

# 提交作业
{baseDir}/scripts/run.sh "提交作业 sleep 900" # 简单命令
{baseDir}/scripts/run.sh "提交作业 启动命令 sleep 900 队列 comp 核心数 2  运行时间 12:00:00 作业名称 production-run1 " # 复杂命令

#提交作业帮助
{baseDir}/scripts/run.sh "如何提交作业"
{baseDir}/scripts/run.sh "提交作业帮助"
{baseDir}/scripts/run.sh "作业有哪些参数"

# 删除作业
{baseDir}/scripts/run.sh "删除作业 12345"

# 作业管理 - 队列和集群
{baseDir}/scripts/run.sh "查询队列"
{baseDir}/scripts/run.sh "集群信息"

# 文件管理 - 列表和上传下载
{baseDir}/scripts/run.sh "文件列表"
{baseDir}/scripts/run.sh "查看文件 /public/home/user"
{baseDir}/scripts/run.sh "上传文件 ./data 到 /public/home/user/"
{baseDir}/scripts/run.sh "下载文件 /public/home/user/data 到 ./"

# 文件管理 - 创建和删除
{baseDir}/scripts/run.sh "创建目录 /public/home/user/workspace"
{baseDir}/scripts/run.sh "创建文件 /public/home/user/data"
{baseDir}/scripts/run.sh "删除文件 /public/home/user/data"

# 文件管理 - 复制移动和重命名
{baseDir}/scripts/run.sh "复制文件 /src/数据 到 /dst/"
{baseDir}/scripts/run.sh "移动文件 /src/数据 到 /dst/"
{baseDir}/scripts/run.sh "重命名 /old/data 为 data1"
{baseDir}/scripts/run.sh "检查文件 /public/home/user/file"

# 账户信息
{baseDir}/scripts/run.sh "查询余额"
{baseDir}/scripts/run.sh "查询用户"
{baseDir}/scripts/run.sh "作业统计"
{baseDir}/scripts/run.sh "机时"

# 区域切换
{baseDir}/scripts/run.sh "切换到山东"
{baseDir}/scripts/Run "切换到西安"

# 缓存管理
{baseDir}/scripts/run.sh "刷新缓存"

# 帮助
{baseDir}/scripts/run.sh "帮助"
```
...
{baseDir}/scripts/run.bat "查看文件 /public/家庭/用户"
{baseDir}/scripts/run.bat "上传文件 ./data 到 /public/家庭/用户/"
{baseDir}/scripts/run.bat "下载文件 /public/家庭/用户/data 到 ./"

# 文件管理 - 创建和删除
{baseDir}/scripts/run.bat "创建目录 /public/家庭/用户/workspace"
{baseDir}/scripts/run.bat "创建文件 /public/家庭/用户/data"
{baseDir}/scripts/run.bat "删除文件 /public/家庭/用户/data"
...
{baseDir}/scripts/run.bat "检查文件 /public/家庭/用户/file"
