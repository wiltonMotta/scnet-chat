...
class IntentRecognizer:
    ...
    def _extract_job_params(self, text: str) -> Dict[str, Any]:
        ...
        # 5. 简洁格式：提交作业 xxx（xxx作为命令，但需排除参数关键词）
        if not cmd:
            text_clean = text
            # 移除区域名称
            for key in self._load_clusters().keys():
                text_clean = text_clean.replace(key, '')
            # 尝试匹配 "提交作业 xxx"，但遇到参数关键词停止
            cmd_match = re.search(r'提交作业\s+([^,，]+)', text_clean)
            if cmd_match:
                potential_cmd = cmd_match.group(1).strip()
                # 如果包含中文参数关键词，可能需要特殊处理
                if any(k in potential_cmd for k in ['队列', '节点', '核心', '运行时间', '作业名', '工作目录']):
                    # 提取第一个逗号/参数前的部分作为命令
                    first_part = re.split(r'[,，]|队列|节点|核心|运行时间|作业名|工作目录', potential_cmd)[0].strip()
                    if first_part and first_part not in ['', 'sleep', 'mpirun', 'python']:
                        cmd = first_part
                else:
                    cmd = potential_cmd
        ...
        if not params.get("nproc"):
            nproc_match = re.search(r'(?:核心数|CPU核心|总核心数)[\s]*[:：]?\s*(\d+)', text)
            if nproc_match:
                params["nproc"] = nproc_match.group(1)
        
        # 提取每节点核心数
        if not params.get("ppn"):
            ppn_match = re.search(r'(?:每节点核心|PPN|每节点)[\s]*[:：]?\s*(\d+)', text)
            if ppn_match:
